...
export default function CreateTemplateElementsList({
  workspaceId,
  templateId,
  onCreated,
}: Props) {
  ...
  const handleCreate =
    ...
    async (e?: MouseEvent<HTMLAnchorElement | HTMLButtonElement>) => {
      ...
      try {
        ...
        // POST to template-elements API instead of page-elements
        const res = await fetch(
          `/api/workspaces/${workspaceId}/templates/${templateId}/elements`,
          {
            method: 'POST',
            body: JSON.stringify({
              element: elementSlug,
        ...
        const id: string | undefined = json?.element?._id;
        if (!id) return;
