...
import { TemplatePublishStatus } from '@/lib/workspaces/templates/templates.types';

export type TemplatesFilterState = {
...
type Props = {
  value: TemplatesFilterState;
  onChange: (next: TemplatesFilterState) => void;
  throttleMs?: number;
};
...
export default function TemplatesFilter({
  ...
  const toggleStatus = (s: TemplatePublishStatus) => {
  ...
  const isActive = (s: TemplatePublishStatus) => value.status.includes(s);
